...
from data.transforms.augmix import AugMix
from data.transforms.rand_augment import RandAugment
...
def _get_augmentation(aug_type: str, aug_paras: Optional[Dict[str, Any]] = None) -> List[Callable]:
    ...
    if aug_type == "default":
        ...
        aug = [
            Permute((1, 0, 2, 3)),
            RandAugment(
                magnitude=aug_paras.get("magnitude", _RANDAUG_DEFAULT_PARAS["magnitude"]),
                num_layers=aug_paras.get("num_layers", _RANDAUG_DEFAULT_PARAS["num_layers"]),
                prob=aug_paras.get("prob", _RANDAUG_DEFAULT_PARAS["prob"]),
                sampling_type=aug_paras.get("sampling_type", _RANDAUG_DEFAULT_PARAS["sampling_type"]),
                sampling_hparas=aug_paras.get("sampling_hparas", _RANDAUG_DEFAULT_PARAS["sampling_hparas"]),
            ),
            Permute((1, 0, 2, 3)),
        ]
        ...
        aug = [
            Permute((1, 0, 2, 3)),
            AugMix(
                magnitude=aug_paras.get("magnitude", _AUGMIX_DEFAULT_PARAS["magnitude"]),
...
def create_video_transform(
    ...
    if isinstance(crop_size, int):
        ...
        assert max(crop_size) <= min_size, "the height and width in crop_size must be less than or equal to min_size"
    else:
        raise TypeError
    ...
    transform = Compose(
        ([] if num_samples is None else [UniformTemporalSubsample(num_samples=num_samples)])
