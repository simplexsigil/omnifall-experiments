...
def _interpolate_opencv(x: torch.Tensor, size: Tuple[int, int], interpolation: str) -> torch.Tensor:
    ...
    if not _HAS_CV2:
        raise ImportError(
            "opencv is required to use opencv transforms. Please " "install with 'pip install opencv-python'."
        )
    ...
    new_h, new_w = size
    img_array_list = [img_tensor.squeeze(0).numpy() for img_tensor in x.permute(1, 2, 3, 0).split(1, dim=0)]
...
def uniform_temporal_subsample_repeated(
    ...
    for ratio in frame_ratios:
        pathway = uniform_temporal_subsample(frames, temporal_length // ratio, temporal_dim)
...
def random_crop_with_boxes(images: torch.Tensor, size: int, boxes: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    ...
    return cropped, clip_boxes_to_image(cropped_boxes, cropped.shape[-2], cropped.shape[-1])
...
def _get_param_spatial_crop(
    ...
    for _ in range(num_tries):
        ...
        if log_uniform_ratio:
            ...
            aspect_ratio = math.exp(log_ratio[0] + torch.rand(1).item() * (log_ratio[1] - log_ratio[0]))
        else:
...
def random_resized_crop(
    ...
    """
    assert scale[0] > 0 and scale[1] > 0, "min and max of scale range must be greater than 0"
    assert aspect_ratio[0] > 0 and aspect_ratio[1] > 0, "min and max of aspect_ratio range must be greater than 0"
    ...
    i, j, h, w = _get_param_spatial_crop(scale, aspect_ratio, height, width, log_uniform_ratio, num_tries)
    ...
    i_, j_, h_, w_ = _get_param_spatial_crop(scale, aspect_ratio, height, width, log_uniform_ratio, num_tries)
