...
import data.transforms.functional
import torch
...
class UniformTemporalSubsample(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        return data.transforms.functional.uniform_temporal_subsample(x, self._num_samples, self._temporal_dim)
...
class UniformTemporalSubsampleRepeated(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        Args:
            x (torch.Tensor): video tensor with shape (C, T, H, W).
        """
        return data.transforms.functional.uniform_temporal_subsample_repeated(x, self._frame_ratios, self._temporal_dim)
...
class ShortSideScale(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x (torch.Tensor): video tensor with shape (C, T, H, W).
        """
        return data.transforms.functional.short_side_scale(x, self._size, self._interpolation, self._backend)
...
class RandomShortSideScale(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return data.transforms.functional.short_side_scale(x, size, self._interpolation, self._backend)
...
class UniformCropVideo(torch.nn.Module):
    ...
    def __call__(self, x: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        Args:
            x (Dict[str, torch.Tensor]): video clip dict.
        """
        x[self._video_key] = data.transforms.functional.uniform_crop(
...
class RandomResizedCrop(torch.nn.Module):
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        return data.transforms.functional.random_resized_crop(
            x,
            self._target_height,
            self._target_width,
            self._scale,
...
class OpSampler(torch.nn.Module):
    ...
    def __init__(
        ...
        self.transforms_prob = torch.FloatTensor(
            transforms_prob if transforms_prob is not None else [1] * len(transforms_list)
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        depth = (
            torch.randint(1, self.num_sample_op + 1, (1,)).item() if self.randomly_sample_depth else self.num_sample_op
        )
...
class Div255(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        return torchvision.transforms.Lambda(data.transforms.functional.div_255)(x)
